...
@Component({
  selector: 'app-cuenta-cobrar-form',
  imports: [ReactiveFormsModule],
  templateUrl: './cuenta-cobrar-form.html',
  styleUrl: './cuenta-cobrar-form.css'
})
export class CuentaCobrarFormComponent implements OnInit {
  ...
  submit(): void {
    ...
    this.tesoreriaService
      ...
      .subscribe({
        next: () => {
          this.toast.success('Cuenta por cobrar registrada.');
          this.router.navigate(['/tesoreria']);
        },
        error: (err: HttpErrorResponse) => {
          const msg = (err.error as ApiError | undefined)?.message ?? 'No se pudo registrar la cuenta por cobrar.';
          this.error.set(msg);
