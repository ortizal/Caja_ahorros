...
async function clicarConCaja(page: Page, boton: Locator): Promise<void> {
  await boton.click();
...
test.describe('Modulo de creditos', () => {
  test('crear producto, solicitar, evaluar, aprobar, desembolsar, cobrar cuota, mora y refinanciar', async ({ page }) => {
    ...
    // Solicitar credito para un socio libre
    ...
    await clicarConCaja(page, filaCredito.getByTestId('btn-desembolsar'));
  ...
  test('auditor solo consulta creditos', async ({ page }) => {
    ...
    await expect(page.getByTestId('solicitud-form')).toHaveCount(0);
  });
